...
export const RoundResultsForm = ({ round, positions, onPositionChange, onSubmit, error, submitting, onSwapPlayer }: RoundResultsFormProps) => {
  const positionValues = round.players.map((p) => Number.parseInt(positions[p.id] || '0', 10))
  const allFilled = positionValues.every((n) => n >= 1 && n <= 24)
  const uniquePositions = new Set(positionValues).size === positionValues.length
  const isValid = allFilled && uniquePositions

  const validationHint = !allFilled ? 'Enter a position (1–24) for every player before submitting.' : !uniquePositions ? 'Each position must be unique.' : ''
