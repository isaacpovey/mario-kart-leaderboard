...
const TrackEntry = ({ stat, rank }: { readonly stat: TrackStat; readonly rank: number }) => (
  <Box p={{ base: 3, md: 4 }} bg="bg.panel" borderRadius="card" borderWidth="1px" borderColor="gray.200" boxShadow="card">
    <HStack justify="space-between" gap={{ base: 2, md: 3 }}>
      <HStack gap={{ base: 2, md: 3 }} flex={1} minW={0}>
        <Badge colorScheme="gray" fontSize={{ base: 'sm', md: 'md' }} px={2} py={0.5} borderRadius="md" fontWeight="bold">
          #{rank}
        </Badge>
        <VStack align="start" gap={0} minW={0}>
          <Text fontWeight="bold" fontSize={{ base: 'sm', md: 'md' }} color="gray.900" truncate>
            {stat.trackName}
          </Text>
      ...
      <Text fontSize={{ base: 'lg', md: 'xl' }} fontWeight="bold" color={getPositionColor(stat.averagePosition)} flexShrink={0}>
...
const TrackSection = ({ title, tracks, startRank }: { readonly title: string; readonly tracks: ReadonlyArray<TrackStat>; readonly startRank: number }) => (
...
export const PlayerTrackStats = ({ trackStats }: PlayerTrackStatsProps) => {
  ...
  const worst = qualifying.length > 3 ? qualifying.slice(-3).reverse() : []

  return (
    <Grid templateColumns={{ base: '1fr', md: '1fr 1fr' }} gap={{ base: 4, md: 6 }}>
      <TrackSection title="Best Maps" tracks={best} startRank={1} />
      {worst.length > 0 && <TrackSection title="Worst Maps" tracks={worst} startRank={qualifying.length - worst.length + 1} />}
    </Grid>
  )
}
