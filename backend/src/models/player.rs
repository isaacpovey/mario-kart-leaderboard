...
impl Player {
    ...
    pub async fn find_by_team_ids(
        ...
        let rows = sqlx::query_as::<_, (Uuid, Uuid, Uuid, String, i32)>(
            "SELECT tp.team_id, p.id, p.group_id, p.name, p.elo_rating
        ...
        Ok(rows.into_iter().map(|(team_id, id, group_id, name, elo_rating)| {
            (team_id, Player { id, group_id, name, elo_rating })
        }).collect())
    }
    ...
    pub async fn find_by_round_keys(
        ...
        let round_numbers: Vec<i32> = round_keys.iter().map(|(_, round_number)| *round_number).collect();

        let rows = sqlx::query_as::<_, (Uuid, i32, Uuid, Uuid, String, i32)>(
            "SELECT rp.match_id, rp.round_number, p.id, p.group_id, p.name, p.elo_rating
        ...
        Ok(rows
            .into_iter()
            .map(|(match_id, round_number, id, group_id, name, elo_rating)| {
                ((match_id, round_number), Player { id, group_id, name, elo_rating })
            })
            .collect())
    }
}
